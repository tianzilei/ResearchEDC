...
> **Last updated:** 2026-06-17 (Legacy code removal is **not complete**. Tracked workflow progress is 920/963 closed, **95.5%**. Phase B schema ownership, Phase C SPI widening, and Phase 1 web/JSP/servlet deletion are complete. Remaining blockers are 43 DAO SPI Java files under `shared/dao`. Phase 3 ledger: 729/878 methods module-backed; 878/878 module-backed or removed, **100.0%**; 0 unused rows remain; 149 removed; 0 fallback-SQL, legacy-only, or adapter-gap rows remain. See `docs/refactor/remove-legacy-code-plan.md`.)
...
```
shared/   246 Java files → bean/ dao/ domain/ job/ exception/ validator/ i18n/ patterns/ core/ log/
           43 Java files under shared/src/main/java/org/researchedc/dao
...
inventory 43 active artifacts -> 43 keep compatibility, 0 unknown
...
| C1-C4 | Legacy code deletion | 🔶 In progress, not complete | **0 direct legacy constructor matches, 0 `DaoProvider.getDao()` calls, 24/24 DAO families SPI-widened. Workflow progress is 920/963 closed (95.5%). Physical deletion remains blocked by DAO implementation replacement proof.** |
