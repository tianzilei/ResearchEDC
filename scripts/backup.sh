#!/bin/bash
# =============================================================================
# OpenClinica — Database & Files Backup
#
...
echo "=== OpenClinica Backup (${TIMESTAMP}) ==="
echo "  Mode:      ${MODE}"
...
DB_FILE="${BACKUP_DIR}/openclinica_db_${TIMESTAMP}.sql"
if docker ps --format '{{.Names}}' | grep -q "^${DB_CONTAINER}$"; then
    ...
    docker cp "${DB_CONTAINER}:/tmp/oc_backup_${TIMESTAMP}.dump" "${BACKUP_DIR}/openclinica_db_${TIMESTAMP}.dump"
    docker exec "${DB_CONTAINER}" rm -f "/tmp/oc_backup_${TIMESTAMP}.dump"
    echo "  DB backup: ${BACKUP_DIR}/openclinica_db_${TIMESTAMP}.dump"
    ...
    docker exec "${DB_CONTAINER}" pg_dump \
        ...
        "${DB_NAME}"
    docker cp "${DB_CONTAINER}:/tmp/oc_schema_${TIMESTAMP}.sql" "${BACKUP_DIR}/openclinica_schema_${TIMESTAMP}.sql"
    docker exec "${DB_CONTAINER}" rm -f "/tmp/oc_schema_${TIMESTAMP}.sql"
    echo "  Schema:    ${BACKUP_DIR}/openclinica_schema_${TIMESTAMP}.sql"
else
...
DATA_FILE="${BACKUP_DIR}/openclinica_data_${TIMESTAMP}.tar"
if docker volume inspect "${DATA_VOLUME}" &>/dev/null; then
    docker run --rm \
        -v "${DATA_VOLUME}:/data:ro" \
        -v "${BACKUP_DIR}:/backup" \
        alpine tar cf "/backup/openclinica_data_${TIMESTAMP}.tar" -C /data .
...
if [ "${COMPRESS}" = true ]; then
    ...
    gzip "${BACKUP_DIR}/openclinica_db_${TIMESTAMP}.dump" 2>/dev/null || true
    gzip "${BACKUP_DIR}/openclinica_schema_${TIMESTAMP}.sql" 2>/dev/null || true
    gzip "${BACKUP_DIR}/openclinica_data_${TIMESTAMP}.tar" 2>/dev/null || true
...
if [ -n "${S3_BUCKET}" ]; then
    ...
    aws s3 cp "${BACKUP_DIR}/openclinica_db_${TIMESTAMP}.dump" "${S3_BUCKET}/db/" || echo "  WARNING: S3 upload failed"
    aws s3 cp "${BACKUP_DIR}/openclinica_data_${TIMESTAMP}.tar" "${S3_BUCKET}/data/" || echo "  WARNING: S3 upload failed"
...
ls -lh "${BACKUP_DIR}/openclinica_"*"${TIMESTAMP}"* 2>/dev/null || true
echo ""
