...
dao_provider_matches="$(rg -n 'DaoProvider\.getDao' app shared web ws -g '*.java' 2>/dev/null || true)"
...
new_jsp_files="$(git ls-files --others --exclude-standard 'web/**/*.jsp' 2>/dev/null || true)"
fail_if_matches "no new untracked JSP files" "${new_jsp_files}"
