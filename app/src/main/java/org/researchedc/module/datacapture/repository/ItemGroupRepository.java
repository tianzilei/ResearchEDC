...
public interface ItemGroupRepository extends JpaRepository<ItemGroupEntity, Integer> {
    ...
    @Query(value = "SELECT ig.* FROM module_item_group ig, item_group_metadata igm, item it"
    ...
    @Query(value = "SELECT DISTINCT ig.* FROM module_item_group ig, item_group_metadata igm"
    ...
    @Query(value = "SELECT ig.* FROM module_item_group ig, item_group_metadata igm,"
            + " section s, item_form_metadata ifm"
            + " WHERE ifm.section_id = ?1"
            + " AND igm.crf_version_id = s.crf_version_id"
            + " AND ig.item_group_id = igm.item_group_id"
