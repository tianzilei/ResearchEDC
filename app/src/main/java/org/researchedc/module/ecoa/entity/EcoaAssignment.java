...
@Entity(name = "ModuleEcoaAssignment")
@Table(name = "ecoa_assignment")
public class EcoaAssignment {
    ...
    @Column(name = "questionnaire_assignment_id", length = 80)
    private String questionnaireAssignmentId;

    @Column(nullable = false, length = 20)
    @Enumerated(EnumType.STRING)
    private EcoaAssignmentStatus status = EcoaAssignmentStatus.PENDING;
    ...
    void onCreate() {
        ...
        if (status == null) {
            status = EcoaAssignmentStatus.PENDING;
        }
    }
