...
| P0-H4 | Frontend has no global 401/403 handling for API client calls. | `ApiClient` throws raw `ApiError`; router has `/app/403`, but request failures are handled ad hoc by pages. `ProtectedRoute` returns `null` while auth initializes. | core-logic | business-logic | frontend/security | Add predictable handling for session expiry and forbidden actions. |
...
2. Add consistent 401/403 frontend handling.
