...
- Slice 2 per-study scoping was extended to import jobs: create, upload, list, get, preview, validate, and commit now use the authenticated user id and require same-study import access. Access-denied/not-found/validation state exceptions escape the import controller's validation/commit catch blocks so the global JSON error envelope is preserved instead of being converted to a generic 500 preview/result.
- Verified import scoping with `mvn test -pl app -am '-Dtest=ImportServiceTest,ImportControllerTest,CurrentStudyAccessServiceTest,ModulithVerificationTest' '-Dsurefire.failIfNoSpecifiedTests=false'` (52 tests passed).
- Slice 2 per-study scoping was extended to datasets: the shared current-study access service now exposes read/write study checks, unfiltered dataset listing is limited to administrator users or the caller's readable study ids, dataset lookup/list-by-study require read access, and create/update require write access with ownership still derived from the authenticated user.
- Verified dataset scoping with `mvn test -pl app -am '-Dtest=DatasetServiceTest,DatasetControllerTest,CurrentStudyAccessServiceTest,ModulithVerificationTest' '-Dsurefire.failIfNoSpecifiedTests=false'` (31 tests passed).
- Slice 2 per-study scoping was extended to discrepancy notes: study listing, event-CRF note listing, lookup, create, and resolve now pass the authenticated user through service-level read/write checks. The `eventCrfId` list path no longer treats the event CRF id as a study id, and compatibility coverage preserves the current data-entry note shape while filtering unreadable studies.
- Verified discrepancy-note scoping with `mvn test -pl app -am '-Dtest=DiscrepancyNoteServiceTest,DiscrepancyNoteControllerTest,CurrentStudyAccessServiceTest,ModulithVerificationTest' '-Dsurefire.failIfNoSpecifiedTests=false'` (29 tests passed).
