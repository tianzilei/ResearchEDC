...
- **Refactor progress snapshot (2026-06-14):** active workflow inventory is 848/963 closed (**88.1%**), Phase 3 DAO method coverage is 822/885 module-backed or removed (**92.9%**), DAO method blockers are 63/885 unused rows (**7.1%**), and DAO-surface file deletion is 98/186 (**52.7%**).
- **Phase 3 ledger status (2026-06-14, updated):** `docs/refactor/phase-3-dao-replacement-ledger.{md,csv}` tracks 885 SPI methods: 757 `module-backed`, 0 `fallback-sql`, 0 `legacy-only`, 0 `adapter-gap`, 63 `unused`, and 65 `removed`. Deletion is now gated by removing the remaining unused SPI rows and proving no registration/factory/inheritance/runtime dependency remains for each implementation/support file.
