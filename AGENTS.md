...
- **Refactor progress snapshot (2026-06-15):** active workflow inventory is 848/963 closed (**88.1%**), Phase 3 DAO method coverage is 856/878 module-backed or removed (**97.5%**), DAO method blockers are 22/878 unused rows (**2.5%**), and DAO-surface file deletion is 98/186 (**52.7%**).
- **Phase 3 ledger status (2026-06-14, updated):** `docs/refactor/phase-3-dao-replacement-ledger.{md,csv}` tracks 878 SPI methods: 758 `module-backed`, 0 `fallback-sql`, 0 `legacy-only`, 0 `adapter-gap`, 50 `unused`, and 70 `removed`. Deletion is now gated by removing the remaining unused SPI rows and proving no registration/factory/inheritance/runtime dependency remains for each implementation/support file.
