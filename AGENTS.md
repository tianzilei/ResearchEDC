...
│   └── module/              # Spring Modulith modules (17 modules, 397 Java files)
│       ├── randomization/   # 随机化系统 (算法 + API, 37 文件)
│       ├── export/          # 导出中心 (异步任务, 9 文件)
│       ├── crf/             # CRF 元数据 (module-owned repositories, adapter retired)
│       ├── audit/           # 审计日志 (事件驱动 + 独立表 + 只读审计查询实体, 48 文件)
...
│   ├── domain/              # retired; no Java files remain
│   ├── bean/                # retired; all DTOs migrated to module-owned DTOs
...
| **CRF module** | `app/.../module/crf/` | CRF 列表/版本/预览, module-owned repositories |
...
- **Quality:** `pnpm typecheck` (0 errors) | `pnpm lint` (0 errors, 0 warnings) | `pnpm test` (25/25 ✅)
...
- `JAVA_HOME` should point to JDK 21, and `javac` 21 must be on `PATH`
- local verification can pin Oracle JDK with `env JAVA_HOME=/usr/lib/jvm/jdk-21.0.11-oracle-x64 PATH=/usr/lib/jvm/jdk-21.0.11-oracle-x64/bin:$PATH <command>`
- PostgreSQL is only needed for DB-backed integration checks; current app unit/modulith suite passes without it
...
- **Import/Export**: REST API (`/api/v1/exports`, `/api/v1/imports/upload`), PostgreSQL dump verified
...
- **NEVER** hardcode file paths - use app/module-owned configuration support
...
mvn test -pl app -am -Dtest=OdmExportGeneratorTest -Dsurefire.failIfNoSpecifiedTests=false
...
- **shared/ Java surface:** `shared/src/main/java` contains **0 Java files**. All `shared/bean` DTOs have been migrated to module-owned DTOs under `app/module/*/dto/`. The `shared/` module now contains only resource files (i18n properties, Liquibase migrations, ODM/XSD templates).

### Legacy DAO Refactor Handoff (2026-06-05)

- **Status:** Phase 3 DAO SPI deletion **COMPLETE** — DAO families were SPI-widened, callers were migrated to module-owned ports, and `shared/src/main/java/org/researchedc/dao` is now empty. `web/` and `ws/` are absent, `LegacyDaoFactory` and `EntityDAO` infrastructure are deleted.
