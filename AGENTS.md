...
├── shared/                  # 共享领域逻辑与数据访问 — 213 Java files (replaces legacy-core, still legacy-heavy)
│   ├── bean/                # DTOs (81 Java files)
│   ├── dao/                 # 数据访问层 (11 SPI interfaces)
...
| Legacy DAOs | `shared/.../dao/` | 11 DAO SPI Java files; deletion blocked by caller migration |
| Legacy DAO SPI interfaces | `shared/.../dao/spi/` | 11 个接口 (IStudyDAO, ...) |
...
- **Status:** Phase C SPI widening **COMPLETE** — DAO families are SPI-widened. The remaining checked-in DAO surface is SPI-only: 11 Java files under `shared/dao/spi`. `web/` and `ws/` are absent, `LegacyDaoFactory` and `EntityDAO` infrastructure are deleted, and remaining deletion is blocked by callers that still depend on legacy SPI names.
...
- **Refactor progress snapshot (2026-06-18):** active workflow inventory is 952/963 closed (**98.9%**), Phase 3 DAO method coverage is 878/878 module-backed or removed (**100%**), DAO method blockers are 0/878 unused rows (**0%**), shared/ reduced from 793 to 213 files (**73.1%**), DAO-surface deletion is 175/186 files (**94.1%**), LegacyDaoFactory eliminated, EntityDAO infrastructure deleted, dead code cleanup complete.
- **Phase 3 ledger status (2026-06-18):** `docs/refactor/phase-3-dao-replacement-ledger.{md,csv}` tracks 878 SPI methods: 341 `module-backed`, 0 `fallback-sql`, 0 `legacy-only`, 0 `adapter-gap`, 0 `unused`, and 537 `removed`. Deletion is now gated by migrating callers from legacy SPI names to module-owned ports.
- **Remaining work:** `shared/dao` is now SPI-only with 11 DAO SPI Java files. Remaining deletion is blocked by callers that still depend on legacy SPI names instead of module-owned ports. Latest caller-migration slices moved audit database-changelog, audit-user-event, study-subject event audit reads, and attachment authorization identity lookups off legacy SPI injection; no-caller SPI deletion has removed obsolete database changelog, measurement-unit, tag, dataset, filter, event-definition CRF, section, dynamics metadata, item-group metadata, discrepancy-note, subject, subject-group, SCD metadata, and identity contracts.
- **Gauntlet commands:**
  - `git status --short`
