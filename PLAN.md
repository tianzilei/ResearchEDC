...
| Java 测试覆盖 | **58 tests** / 1300+ files (study 14, randomization 25, export 11, modulith 1) | 🟡 3 个模块已达基线 |
...
- [x] **Modulith 模块测试规范**: 3 个模块 (study, randomization, export) 已有 Service 层单元测试
  - 单元测试：JUnit 5 + Mockito，不依赖数据库 ✅ (StudyService: 14, RandomizationService: 11, ExportService: 11)
  - 控制器测试：RandomizationController 有 9 个 MockMvc 测试
  - 模块边界测试：`ModulithVerificationTest` 验证无非法依赖 ✅
  - 待覆盖模块: subject, event, datacapture, identity
...
- [x] **Modulith 模块测试覆盖确认**: 58 tests, 0 failures (JDK 21)
