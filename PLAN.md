...
- [ ] **Dependabot / Renovate 配置** — 自动提交依赖升级 PR
  - Spring Boot / Hibernate / Jackson 等核心框架：自动合并补丁版本（patch），手动评审小版本（minor）
  - 遗留依赖（Ehcache、Quartz）标记为 `skip` 直到迁移完成
- [ ] **Maven BOM `openclinica-dependencies`** — 创建项目级依赖清单 BOM
  - 将全项目允许的依赖及其版本集中管理，避免子模块各自引入不同版本
  - 继承 Spring Modulith BOM，增加项目特有的版本锁定
...
- [ ] **Commons Collections 3 → 4**: 包名 `org.apache.commons.collections` → `org.apache.commons.collections4`（注意 `Bag`/`Buffer` 等类在 v4 中可能被移除，需评估使用场景）
- [ ] **Javassist 3.8 移除**: Hibernate 6 自带 ByteBuddy，检查 Javassist 是否被显式引用后移除
- [ ] **Maven Enforcer 禁用规则**: 配置 `bannedDependencies` 禁止新模块引入 `commons-lang:commons-lang` 和 `commons-collections:commons-collections`

#### 类型 B：可替换但成本高（Phase 1–3 分阶段替换）

- [ ] **Quartz 1.8 → 2.3**: API 变化较大（`JobDetailBean` → `JobDetailImpl`，`CronTriggerBean` → `CronTriggerImpl`）
...
- [ ] **Ehcache 2 → Caffeine**: （Phase 3 执行，详见 6.3 节）
  - 如果仅用于 Hibernate 二级缓存，迁移至 Ehcache 3（JCache 实现）
  - 如果用于应用级缓存，Caffeine 是更轻量的现代选择
  - 过渡期：自定义 Spring Cache Manager 同时支持两种缓存，按模块切换

#### 类型 C：无现代替代品但可移除（随 JSP/SOAP 绞杀自然退役）

- [ ] **JMesa**: 遗留 JSP 表格渲染库，标记为 "仅 legacy-web 使用"
  - 随 JSP 页面被 React 取代后直接移除，无需替换
- [ ] **Castor / XmlSchema**: 仅用于旧版 WSDL/SOAP 绑定
  - 随 WS 模块退役时一并移除

#### 类型 D：深度耦合难以替换（建立依赖沙箱隔离）

- [ ] **遗留依赖沙箱**: 在 `legacy-core` 模块的 `pom.xml` 中显式列出所有遗留依赖，添加注释说明 "Legacy Only"
- [ ] **Maven Enforcer `bannedDependencies`**: 配置规则防止 `app/module/*` 意外引入遗留库
- [ ] **依赖影响范围审计**: 分析每个遗留依赖的实际使用范围（仅 core 使用？还是 web/ws 也依赖？）
...
- [ ] **Modulith 模块测试规范**: 每个 `app/module/*` 必须包含：
  ...
  - 模块边界测试：Spring Modulith `ApplicationModules` 验证无非法依赖
- [ ] **Maven Failsafe 配置**: 分离单元测试（surefire）和集成测试（failsafe）
- [ ] **测试数据工厂**: 引入 `instancio` 或自定义 `TestDataFactory`，避免冗长的对象构造代码
- [ ] **Testcontainers 标准化**: 统一使用 PostgreSQL 容器，替代废弃的 DBUnit 和 H2（H2 SQL 方言差异可能导致测试通过但生产失败）
...
- [ ] **功能开关设计**: 在 Study 级别配置 `use_new_data_entry_ui: true`，支持金丝雀迁移
  ...
  - 开关通过 `legacy-gateway` API 暴露，React 路由根据开关值动态渲染
  - 监控两组用户的错误率和完成时间，数据驱动开关全量开启决策

#### 5.2.2 Hybrid Shell 过渡架构

在迁移完成之前，需要一个过渡架构让用户体验"看起来是统一的"：

- [ ] **React App Shell（全局外壳）**: `AppLayout`（顶栏 + 侧栏 + 内容区）作为统一容器
  - 已迁移的页面直接渲染 React 路由组件
  - 未迁移的 JSP 页面通过 `<iframe src="/legacy/xxx">` 嵌入 React 内容区
  - iframe 与父页面通过 `postMessage` 进行必要通信：
    - Study 切换通知 → iframe 内 JSP 刷新上下文
    - 登录状态同步 → 避免重复登录
    - 全局 loading/error 状态传递
...
- [ ] **前端独立部署**: Nginx 直接 serve `frontend/dist/`，不再打包进 WAR
  - `openclinica-api.war`（或 JAR）：仅 Java 后端，目标 < 150 MB
  - `openclinica-frontend`：静态 HTML/JS/CSS 文件，部署到 Nginx 或 CDN
- [ ] **Nginx 路由调整**:
  - `/app/*` → serve `index.html`（React SPA fallback）
  - `/api/*` → proxy 到 Java 后端
  - `/legacy/*` → proxy 到遗留 Tomcat（过渡期）
- [ ] **独立部署收益**: 前端可以独立部署、独立回滚、独立扩容（CDN 缓存），不再受 Java 构建周期约束

#### 方案 B：嵌入式但解耦构建（过渡方案）

如果组织短期内无法接受独立部署，至少在构建流程上解耦：

- [ ] **前端 CI 独立构建**: 前端在 CI 中独立构建，产物上传到 Artifactory/S3
- [ ] **Maven 下载产物**: 通过 `maven-dependency-plugin` 下载前端产物，而不是在 Dockerfile 中执行 `pnpm build`
- [ ] **解耦收益**: 前端团队可以在不触发 Java CI 的情况下完成部署和回滚

#### 制品管理

- [ ] **WAR 体积优化**: 通过 `mvn dependency:analyze` 移除未使用依赖，目标 < 150 MB
- [ ] **遗留镜像收敛**: 收敛为两个运行时单元
  - `openclinica-app`：Spring Boot WAR（core + web + ws + app + 新模块），唯一长期维护的 Java 运行时
  - `openclinica-legacy-web`：过渡期保留，承载尚未迁移的 JSP 页面；JSP 全部绞杀后退役
- [ ] **WS 模块保留**: SOAP 服务如果仍有外部消费者，保留在 `openclinica-app` 中而非单独部署；消费者迁移到 REST 后移除
