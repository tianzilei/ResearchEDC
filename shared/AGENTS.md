...
| DAO deletion blocked | 🔶 `DaoProvider` removed; direct `new XxxDAO(...)` / `new StudyConfigService(...)` matches are 0. Target DAO families are SPI-widened, but 73 DAO/SPI/support Java files still exist under `shared/dao`. Phase 3 ledger: 759/878 methods are module-backed; 878/878 are module-backed or removed (100%); 0 unused rows remain; 119 rows are removed; 0 fallback-SQL, legacy-only, or adapter-gap rows remain. |
