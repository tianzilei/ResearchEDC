...
| DAO deletion blocked | 🔶 `DaoProvider` removed; direct `new XxxDAO(...)` / `new StudyConfigService(...)` matches are 0. Target DAO families are SPI-widened, but 88 DAO/SPI/implementation files still exist. Phase 3 ledger: 757/885 methods are module-backed; 822/885 are module-backed or removed (92.9%); 63 unused rows remain (7.1%); 65 rows are removed; 0 fallback-SQL, legacy-only, or adapter-gap rows remain. |
