...
| DAO deletion blocked | 🔶 `DaoProvider` removed; direct `new XxxDAO(...)` / `new StudyConfigService(...)` matches are 0. Target DAO families are SPI-widened, but 88 DAO/SPI/implementation files still exist. Phase 3 ledger: 756/885 methods are module-backed; 64 unused rows remain; 65 rows are removed; 0 fallback-SQL, legacy-only, or adapter-gap rows remain. |
