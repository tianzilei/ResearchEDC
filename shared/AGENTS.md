...
| DAO deletion blocked | 🔶 `DaoProvider` removed; direct `new XxxDAO(...)` / `new StudyConfigService(...)` matches are 0. Target DAO families are SPI-widened, but 88 DAO/SPI/implementation files still exist. Phase 3 ledger: 757/878 methods are module-backed; 822/878 are module-backed or removed (93.6%); 56 unused rows remain (6.4%); 65 rows are removed; 0 fallback-SQL, legacy-only, or adapter-gap rows remain. |
