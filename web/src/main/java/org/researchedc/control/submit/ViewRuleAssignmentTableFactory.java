...
import org.researchedc.domain.rule.action.RuleActionRunBean;
...
public class ViewRuleAssignmentTableFactory extends AbstractTableFactory {
    ...
    private class ActionSummaryCellEditor implements CellEditor {
        ...
        private void appendDestProps(HtmlBuilder builder,
                List<org.researchedc.domain.rule.action.PropertyBean> propertyBeans) {
            if(propertyBeans!=null && propertyBeans.size()>0) {
                String s = "";
                for(org.researchedc.domain.rule.action.PropertyBean p : propertyBeans) {
                	if(p.getOid()!=null)
                	{
                		s += p.getOid().trim() + ", ";
                	}
        ...
        private void appendStratificationFactors(HtmlBuilder builder,
                List<org.researchedc.domain.rule.action.StratificationFactorBean> factorBeans) {
            if(factorBeans!=null && factorBeans.size()>0) {
                String s = "";
                for(org.researchedc.domain.rule.action.StratificationFactorBean p : factorBeans) {
