-- =============================================================================
-- OpenClinica — PostgreSQL Initial Schema
--
...
-- The full schema is managed by Liquibase migrations in core/src/main/resources/migration/.
--
-- To restore from a production backup instead:
--   1. Place the dump file at ./initdb/openclinica_dump.sql
--   2. It will auto-execute if named *.sql or *.sql.gz
--   3. Or restore manually:
--      docker exec -i oc-postgres psql -U openclinica -d openclinica < dump.sql
-- =============================================================================
...
-- CREATE SCHEMA IF NOT EXISTS openclinica;
-- SET search_path TO openclinica;

-- Quartz scheduler tables are created via Liquibase migrations (oc_qrtz_* tables).
-- They are defined in core/src/main/resources/migration/.
